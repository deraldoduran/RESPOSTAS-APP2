...
CREATE VIEW RESP24
AS
SELECT AU.NOTA AS NOTA, DIS.NOME AS DISCIPLINA, ALU.NOME AS ALUNO, DIS.QUANTCREDITO AS CREDITOS
FROM ALUNOS ALU, AULA AU, DISCIPLINAS DIS, CURSOS CUR, MATRICULA MA, CONTEM CO
WHERE AU.DISCIPLINA = DIS.NUMDISP AND CO.IDCURSO = CUR.NUMCURSO 
AND CO.IDDISCIPLINA = DIS.NUMDISP  AND AU.ALUNO = ALU.NUMALUNO
AND ALU.NOME = 'Edvaldo Carlos Silva' AND AU.NOTA>=7.0
GROUP BY ALU.NOME, DIS.NOME, AU.NOTA, DIS.QUANTCREDITO;
```
```SQL
CREATE VIEW RESP25
AS
...
AND AU.NOTA>=7.0
GROUP BY ALU.NOME, DIS.NOME, AU.NOTA, DIS.QUANTCREDITO
HAVING SUM(DIS.QUANTCREDITO)>= 10;
```
```SQL
CREATE VIEW RESP26
AS
...
AS
SELECT * FROM MEDIAS
WHERE MEDIA_DISCIPLINAS IN (SELECT MIN(MEDIA_DISCIPLINAS) FROM MEDIAS);
```
```SQL
CREATE VIEW MEDIAS
AS
